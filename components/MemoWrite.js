...
const MemoWrite = ({ onClose }) => {
  ...
  const saveMemo = async () => {
    ...
    const updateMemo = [...memos, newMemo]; //만든 storage에 계속 추가되는것이다. 
    setMemos(updateMemo);
    console.table(memos);
  }
  ...
  return (
    <View style={styles.container}>
        ...
        <TextInput 
           ...
           style={[styles.input, styles.multilineInput]} //react-native에서 style 2개를 쓸 땐 [] 배열로 해준다.
        />      
...
const styles = StyleSheet.create({ //react-native에서 style 만드는 법!!
    container: {
        flex: 1,
        alignItems: 'center',
